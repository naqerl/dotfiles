...
** Emacs
#+begin_src conf
$emacs = $HOME/.local/bin/emacsclient -a "" -c
$terminal = $HOME/.local/bin/emacsclient -a "" -c -e "(progn (setq current-prefix-arg '(new)) (call-interactively 'eshell))"
...
** Start apps
#+begin_src conf
exec-once = [workspace 1 silent] bash -c 'emacs -daemon && emacsclient -c -a ""'
...
bind = $mainMod, M, exec, [float; size 40% 40%] foot pulsemixer
bind = $mainMod, D, exec, 'bash ~/.config/bin/toggle-laptop-display'
#+end_src
