...
- Check spell using treesitter and ispell/aspell. The key is to check only custom named nodes (like class, function, variable, string and etc.). Make it independent from emacs to use as pre-commit hook or in any CI/CD pipeline
...
(use-package typescript-mode :disabled)
...
(use-package
 compile-eslint
 :vc (:url "https://github.com/Fuco1/compile-eslint")
...
(use-package
 js
 :after prettier-js
 :bind (:map js-mode-map ("C-c C-f" . prettier-js)))
#+end_src
...
(with-eval-after-load 'org
  ...
  (add-to-list 'org-structure-template-alist '("pu" . "src plantuml :file ")))
#+end_src
...
(use-package
 eshell
 :hook
 (eshell-first-time-mode . my/configure-eshell)
...
(use-package
 ...
 (setq popper-reference-buffers
       '("\\*Async Shell Command\\*" shell-mode eshell-mode eat-mode))
 (setq popper-group-function #'popper-group-by-project) ; group by perspective
 (popper-mode +1)
