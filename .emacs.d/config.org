  ...
  (let ((buffer (get-buffer name)))
    (if buffer
        (with-current-buffer buffer
          (seq-some (lambda (window) (eq (window-buffer window) (current-buffer)))
                (window-list)))
      nil)))

(defun my/next-error ()
  "Navigates to the next xref or flycheck."
  (interactive)
  (if (seq-some #'my/window-with-name-visible-p '("*xref*" "*Occur*"))
      (next-error))
      (flycheck-next-error))


(defun my/previous-error ()
  "Navigates to the previous xref or flycheck."  
  (interactive)
  (if (seq-some #'my/window-with-name-visible-p '("*xref*" "*Occur*"))
      (previous-error))
